...
export default class FeedreScreen extends React.Component {
  ...
  // 서버와 DB 데이터 통신
  async getData() {
    // getData 함수 call시 isLoading값 false로 바꾸어 로딩화면 표시
    let image_list = [];
    try {
      let data = await fetch(
        `${url}/feed/0/${this.state.sorting_price}/'${this.condition}'/'${this.room}'/${this.price}/${this.state.length}`
      );
  ...
  componentDidMount = async () => {
    ...
    const realdata = await fetch(
      `${url}/count/0/'${this.condition}'/'${this.room}'/${this.price}`
    );
